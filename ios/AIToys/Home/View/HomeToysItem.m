...
-(void)setIsEdit:(BOOL)isEdit{
    ...
    if(isEdit){
        ...
        self.rankView.hidden = self.index > 4;
...
-(void)setModel:(HomeDollModel *)model{
    ...
    [self.bgImgView sd_setImageWithURL:[NSURL URLWithString:model.dollModel.backgroundImg] placeholderImage:[UIImage imageNamed:@"home_toys_bg.png"]];
    [self.toysImgView sd_setImageWithURL:[NSURL URLWithString:model.dollModel.coverImg] placeholderImage:[UIImage imageNamed:@"home_toys_img.png"]];
    ...
    NSString *timeStr = @"";
    if(hrs > 0){
        timeStr = [NSString stringWithFormat:@"%ldhrs ",(long)hrs];
    }
    if (mins > 0){
        timeStr = [timeStr stringByAppendingFormat:@"%ldmins ",(long)mins];
    }
    if (s > 0){
        timeStr = [timeStr stringByAppendingFormat:@"%lds ",(long)s];
    }
