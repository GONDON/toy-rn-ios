...
- (UIView *)setupfooterView {
    ...
    [inviteBtn setTitle:@"重新邀请" forState:0];
    ...
    [revokeBtn setTitle:@"撤销" forState:0];
...
- (void)showAlertWithTextField {
    WEAK_SELF
    // 创建UIAlertController
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"名称" message:@"" preferredStyle:UIAlertControllerStyleAlert];
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if(self.memberModel){
            ...
            requestModel.name = self.memberModel.name;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == 0){
        ...
        switch (indexPath.row) {
            case 0:
                cell.titleLabel.text = @"关联账号";
                cell.nameLabel.text = self.memberModel? self.memberModel.userName: self.inviteModel.dealStatus == ThingHomeStatusPending ?@"待加入" : @"已拒绝";
                break;
            case 1:
                cell.titleLabel.text = @"家庭角色";
                cell.nameLabel.text = @"普通成员";
                break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 0 && self.isOwner){
